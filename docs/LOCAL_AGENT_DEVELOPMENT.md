...
Web development mode uses deterministic demo scan data and an in-memory demo Provider so every page and dialog can be visually tested without touching the computer. Overview process rows support name/CPU/memory sorting, larger CPU and memory Top 5 charts, local Agent prompts with registered quit and force-quit operations, name/PID clipboard actions, and user-process termination. The process list holds its current snapshot while the pointer or keyboard focus is inside the panel, so the live monitor cannot move an action target mid-interaction. The main process revalidates the live PID, owner, and system path before registering or executing graceful or force termination; system and other-user processes remain protected.
