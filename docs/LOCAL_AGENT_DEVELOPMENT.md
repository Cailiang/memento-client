...
CC Switch is optional and is read only after the user selects its Settings action. `cc-switch-import.ts` checks the Tauri store for an `app_config_dir_override`, then falls back to `~/.cc-switch/cc-switch.db`. It opens the database read-only and reads only structurally complete Claude, Codex, and Gemini rows. Both importers produce deterministic IDs and use the same endpoint/model/key synchronization, making imports idempotent and preventing duplicates of matching manual providers. Imported plaintext remains in the main process and is immediately normalized and encrypted with AES-256-GCM; the active model-list validation described above applies specifically to the local Claude, Codex, Gemini, and Grok scan.
