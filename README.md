...
Storage cleanup groups only known rebuildable cache paths for Claude, Codex, Antigravity, and Grok; those automatic cache groups do not include credentials, settings, conversations, sessions, workspaces, or projects. A separate review-only scan checks hidden items directly under Home and one level below `.config`, `.cache`, and `.local/share`. It keeps at most 40 items that have not changed for at least 30 days and do not clearly match the installed app inventory, while protecting shell, credential, package-manager, and container roots. Because a command-line tool may still own an unmatched item, Memento never calls it confirmed orphaned data: the user must verify the path, and cleanup revalidates the item before moving it to Trash. Large files at least seven days old and 500 MB in Downloads, Desktop, or Movies follow the same review-and-Trash boundary.
