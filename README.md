...
Official and proxied Google Gemini endpoints use the native `/v1beta` protocol and `x-goog-api-key` header, so credentials never need to appear in a `/models` query string. The connection probe requires a real tool call and tool-result continuation. Gemini 3 probes request the supported `LOW` thinking level instead of the unsupported `MINIMAL` level, while keeping enough output budget for the function call.
