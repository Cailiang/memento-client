...
- **Overview:** inspect health, CPU, GPU, memory, battery, disk, network, and high-usage processes in one quiet live workspace. Sort processes by name, CPU, or memory, view CPU/memory Top 5 charts, ask the local Agent about a process, copy its name/PID, and quit only non-system processes with a revalidated graceful or force action. The process list holds its current order while you interact with it so live refreshes do not move the action menu.
