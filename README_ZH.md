...
存储空间中的数量表示确定性安全规则筛选出的可执行建议，不是磁盘文件总数。“磁盘浏览”会单独异步运行可取消的 `du` 主数据卷扫描，把不小于 5 MB 的目录和文件按容量排序为分栏层级；进度只显示真实耗时、当前位置、项目数和无权限位置数，不估算虚假百分比。磁盘浏览项目可以在 Finder 中定位，但不会自动变成清理操作。
