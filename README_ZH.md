...
- **概览：** 在一个安静的实时工作台中查看健康度、CPU、GPU、内存、电池、磁盘、网络和高占用进程；进程支持按名称、CPU、内存排序，展示更大的 CPU/内存 Top 5 饼图，询问本地 Agent 后会给出需要确认的退出或强制退出操作，也可复制进程名称或 PID；仅非系统进程可执行结束操作，主进程会重新校验。进入列表或打开操作菜单后会暂存当前顺序，实时刷新不会把操作入口移走。
