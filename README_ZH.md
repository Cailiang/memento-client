...
- **电脑体检：** 分开呈现已验证的安全清理、有证据的可行动问题与弱审查线索；同时支持磁盘占用、后台服务和终端启动诊断。
...
应用启动后直接进入确定性电脑体检。查看可信发现、确认并执行直接操作、在维护账本审计结果都不需要模型。只有需要 AI 解释或生成计划时，才在**设置**中配置 OpenAI、Anthropic、Google Gemini、DeepSeek、Grok/xAI、Antigravity 或自定义 OpenAI 兼容地址。
