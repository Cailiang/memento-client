...
存储清理会组合 Claude、Codex、Antigravity 和 Grok 已知的可重建缓存路径；这些自动缓存组不包含密钥、设置、对话、会话、工作区或项目。另一组需要人工确认的扫描会检查 Home 根目录隐藏项目及 `.config`、`.cache`、`.local/share` 的一级子项目，最多保留 40 个至少 30 天未修改、且无法与当前应用清单明确匹配的候选项，同时保护 shell、凭据、包管理器和隐藏容器根目录。由于未匹配项目仍可能属于命令行工具，Memento 不会把它宣称为已确认残留；用户必须核对路径，执行时也会重新校验并且只移到废纸篓。下载、桌面和影片目录中超过 500 MB 且至少 7 天未修改的大文件遵循同样的人工确认与废纸篓边界。
