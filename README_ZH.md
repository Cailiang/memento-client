...
首次执行本机配置扫描时，Memento 会读取 `~/.claude`、`~/.codex`、`~/.gemini` 和 `~/.grok` 中的 API 配置，并通过供应商的只读模型列表校验密钥、服务地址和指定模型。损坏、不完整、认证失败、只有 OAuth、使用不受支持会话令牌或模型不可用的配置会被过滤；再次扫描也会移除先前错误导入且仍未通过校验的本机配置。Memento 不要求安装 CC Switch，也不会自动读取它；“导入 CC Switch”是需要用户主动触发的可选操作。
