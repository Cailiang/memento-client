...
存储清理会组合 Claude、Codex、Antigravity 和 Grok 已知的可重建缓存路径；这些自动缓存组不包含密钥、设置、对话、会话、工作区或项目。另一组需要人工确认的扫描会检查 Home 根目录隐藏项目及 `.config`、`.cache`、`.local/share` 的一级子项目，最多保留 40 个至少 30 天未修改、且无法与当前应用清单或命令身份明确匹配的候选项；命令身份来自 `PATH`、Homebrew 和常见用户 bin 目录，同时 shell、凭据、包管理器和隐藏容器根目录继续受到保护。已知配置会提供经过维护的确定性归属，例如 `.lingma` 会明确显示为阿里云「通义灵码」智能编码助手的配置目录；未知项目仍不会被宣称为已确认残留。用户必须核对路径，执行时也会重新校验并且只移到废纸篓。下载、桌面和影片目录中超过 500 MB 且至少 7 天未修改的大文件遵循同样的人工确认与废纸篓边界。
...
会话 ID、焦点实体和待确认计划会保存在本机。顶部任务标签按会话 ID 管理，而不是按每轮模型运行管理；“这个服务”等后续指代会留在同一标签并继续绑定上一轮的精确对象，不会重新罗列无关项目。分析具体存储项或后台服务时，Agent 会先根据扫描证据直接说明产品、厂商和用途；无法确认时明确说明未知。单独分析应用时会携带精确 Bundle ID、路径、可执行文件、后台组件标记和 URL 协议。应用语言统一控制所有 Agent 可见内容，切换语言后会先刷新体检快照。
...
UI 冒烟测试会在 `1440x900`、`1024x768`、`820x1180` 和 `390x844` 四种视口检查全部五个页面与横向溢出，并覆盖可见构建版本、紧凑页面控件、独立 Agent 会话、同会话追问标签、已知配置归属、约三秒的存储直接操作反馈、Agent 与计划执行进度、结构化结果、磁盘浏览连续删除、任务搜索与删除、应用筛选与忽略、全英文输出、计划确认、体检标签页和 Antigravity 供应商编辑器。真实 Electron 冒烟测试会另外验证生产 preload、真实应用列表、本地化名称、受保护系统应用和真实 Logo。
