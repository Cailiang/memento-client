...
- **Agent：** 用自然语言描述目标，把并行分析保留为状态独立、可以切换的会话标签；同一会话的连续追问不会新增标签。可以直接操作结构化模块结果，再检查计划、确认执行并复检结果。
...
- **任务记录：** 搜索、查看、全选当前筛选结果，或一次确认后批量永久删除保存在本机的 Agent 任务及其工具调用记录。
...
存储清理会组合 Claude、Codex、Antigravity 和 Grok 已知的可重建缓存目录；这些自动缓存组不包含密钥、设置、对话、会话、工作区或项目。另一组需要人工确认的扫描会检查 Home 根目录隐藏目录、`.config`、`.cache`、`.local/share` 的一级子目录，以及 `~/Library/Application Support` 的一级应用数据目录，最多保留 80 个至少 30 天未修改、且无法与当前应用清单或命令身份明确匹配的候选项；命令身份来自 `PATH`、Homebrew 和常见用户 bin 目录，同时 shell、凭据、包管理器、Apple 平台共享目录和隐藏容器根目录继续受到保护。自动清理建议只到目录级：下载、桌面、影片中的个人文件以及 Docker 虚拟磁盘等单文件只会出现在磁盘浏览中。用户必须核对候选目录，执行时也会重新校验并且只移到废纸篓。

存储空间中的数量表示确定性安全规则筛选出的可执行建议，不是磁盘文件总数。“磁盘浏览”会单独异步运行可取消的 `du` 主数据卷扫描，把不小于 5 MB 的目录和文件按容量排序为分栏层级；进度只显示真实耗时、当前位置、项目数和无权限位置数，不估算虚假百分比。本轮扫描注册的磁盘目录可以在 Finder 中定位、确认后移到废纸篓，或通过右键“询问 AI”判断归属和是否能删。Agent 只接收主进程解析后的不透明节点焦点和浅层证据，不接受渲染层提交的清理路径；确认是卸载残留时，才会把已注册的废纸篓操作提供给用户确认。移动成功后会立即移除该子树并扣减可见祖先容量，同时保留同轮扫描中其他项目的 ID，因此可以连续删除多个项目。完整磁盘扫描只在用户明确发起时运行，不再每删一项就自动重扫。

在存储建议中直接执行清理时，主进程仍会验证注册操作和真实目标；成功后界面用约 3 秒的“执行、更新、完成”反馈移除当前候选项，不再等待完整电脑体检。后台服务、终端修复和 Agent 确认计划仍会重新体检验证。
...
会话 ID、焦点实体和待确认计划会保存在本机。顶部任务标签按会话 ID 管理，而不是按每轮模型运行管理；“这个服务”等后续指代会留在同一标签并继续绑定上一轮的精确对象，不会重新罗列无关项目。聚焦分析会按精确身份 token 动态关联其他扫描模块、受限目录文件名、软件包收据、目标目录浅层结构、已脱敏的 shell 引用和已注册终端发现。分析服务时，关联的可修复配置问题也会出现在同一结果中，例如 `.zshrc` 里指向旧版本的字面量 `POSTGRESQL_HOME`，用户可以单独加入当前计划。Agent 会区分“本机确认”“明确签名”和“未确认”证据；可以用模型通用知识解释产品，但关于本机状态的结论必须来自现场证据。单独分析应用时会携带精确 Bundle ID、路径、可执行文件、后台组件标记和 URL 协议。应用语言统一控制所有 Agent 可见内容，切换语言后会先刷新体检快照。

后台服务按残留启动项、启动异常、CPU 占用异常、内存占用异常、长期运行、长期未使用和其他服务分类。健康评分会直接显示待查看模块及其中的项目数，并打开可执行建议最多的完整模块视图，不再跳进项目很少的异常子分类。
...
8. Agent 计划、服务和终端操作完成后重新体检；存储页直接清理依据主进程的真实执行结果更新当前列表。每个操作的结果都会保存。
...
UI 冒烟测试会在 `1440x900`、`1024x768`、`820x1180` 和 `390x844` 四种视口检查全部五个页面与横向溢出，并覆盖可见构建版本、紧凑页面控件、独立 Agent 会话、同会话追问标签、关联配置分析、可操作体检摘要、CPU/内存服务分类、约三秒的存储直接操作反馈、Agent 与计划执行进度、结构化结果、磁盘浏览连续删除与目录询问 AI、任务筛选与批量删除、应用筛选与忽略、全英文输出、计划确认、体检标签页和 Antigravity 供应商编辑器。真实 Electron 冒烟测试会另外验证生产 preload、真实应用列表、本地化名称、受保护系统应用和真实 Logo。
