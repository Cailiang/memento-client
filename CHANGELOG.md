...
- Upgraded artifact upload and download steps to `actions/upload-artifact@v7` and `actions/download-artifact@v8`.
- Removed the remaining Node.js 20 action-runtime deprecation annotations from all six platform build jobs.
- Retained the verified eight-package checksum manifest and cross-platform release matrix unchanged.
- Updated release metadata and operator documentation for the current GitHub Actions artifact runtime.

### 简体中文

- 将产物上传与下载步骤升级到 `actions/upload-artifact@v7` 和 `actions/download-artifact@v8`。
- 移除六个平台构建任务中剩余的 Node.js 20 action 运行时弃用注解。
- 保持已验证的 8 安装包校验清单与跨平台发布矩阵不变。
- 同步更新当前 GitHub Actions 产物运行时对应的版本元数据和操作文档。
...
- Upgraded the release artifact download action to a Node.js 24-based major version, removing the deprecation warning from the publishing job.
...
- 将发布产物下载 action 升级到基于 Node.js 24 的主版本，消除发布任务中的 Node.js 20 弃用警告。
